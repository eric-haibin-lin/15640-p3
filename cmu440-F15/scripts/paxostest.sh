...
# Exit immediately if there was a compile-time error.
go install github.com/cmu440-F15/paxosapp/tests/paxostest
if [ $? -ne 0 ]; then
   echo "FAIL: code does not compile"
   exit $?
fi
...
# Start paxos node.
${PAXOS_NODE} -ports=${ALL_PORTS} -N=3 -id=0 -pxport=${PROXY_PORT} -proxy=0 2 &
PAXOS_NODE_PID0=$!
sleep 1

${PAXOS_NODE} -ports=${ALL_PORTS} -N=3 -id=1 -pxport=${PROXY_PORT} -proxy=0 2 &
PAXOS_NODE_PID1=$!
sleep 1

${PAXOS_NODE} -ports=${ALL_PORTS} -N=3 -id=2 -pxport=${PROXY_PORT} -proxy=0 2 &
PAXOS_NODE_PID2=$!
sleep 1
